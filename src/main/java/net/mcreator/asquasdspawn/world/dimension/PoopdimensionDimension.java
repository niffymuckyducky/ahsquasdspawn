...
package net.mcreator.asquasdspawn.world.dimension;
...
import net.mcreator.asquasdspawn.item.PoopdimensionItem;
import net.mcreator.asquasdspawn.block.PoopportalblocBlock;
...
public class PoopdimensionDimension extends AsquasdspawnElements.ModElement {
	@ObjectHolder("asquasdspawn:poopdimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("asquasdspawn:poopdimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public PoopdimensionDimension(AsquasdspawnElements instance) {
		super(instance, 10);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("poopdimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("asquasdspawn:poopdimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("asquasdspawn:poopdimension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("asquasdspawn:poopdimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("asquasdspawn:poopbiome")),};
	}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == PoopportalblocBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == PoopportalblocBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == PoopportalblocBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag ? PoopportalblocBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, PoopportalblocBlock.block.getDefaultState().getBlock().getDefaultState(),
								3);
					}
				}
			}
